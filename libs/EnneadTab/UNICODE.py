...
def convert_unicode_to_string(unicode_string):
    ...
    while True:
        if attempt > max_attemp:
            NOTIFICATION.messenger(main_text="Cannot convert unicode to string----C!!!")
        ...
        if not data.get("is_ready", False):
            ...
            sleep(0.1)
            continue
        NOTIFICATION.messenger(main_text="Convertion done!!!!")
...
def action_converter():
    ...
    while True:
        if attempt > max_attemp:
            NOTIFICATION.messenger(main_text="Convertion data saved done--A!!!!")
            return
    ...
    NOTIFICATION.messenger(main_text="Convertion data saved done---B!!!!")
    DATA_FILE.save_dict_to_json_in_dump_folder(data, UNICODE_DATA_FILE, use_encode=True)
