...
__doc__ = "In the revit file with copy/monitor status, compare how many levels and grids have two way monitor status and how many have one way monitor.\n\nThis usually happens when somewhere in the life cycle one side the element is recreated/deleted or massively renamed, which means other link file need to catchup. This takes you further than the native copy/monitor review."
__title__ = "33_check monitoring status(Site File only)"
...
def get_level_by_name(name):
    grids = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Levels).WhereElementIsNotElementType().ToElements()
    return filter(lambda x: x.Name == name, grids)[0]
...
def get_main_grids_in_doc(doc):
    grids = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Grids).WhereElementIsNotElementType().ToElements()
    return element_in_primary_option(grids)

def get_levels_in_doc(doc):
    return DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Levels).WhereElementIsNotElementType().ToElements()

def element_in_primary_option(list):
    OUT = []
    for element in list:
...
for revit_link in revit_links:
    ...
    levels_in_links.extend(get_levels_in_doc(link_doc))
...
link_level_names = [x.Name for x in levels_in_links]
# print my_grid_names
# print link_grid_names

grids_in_site_but_not_in_links = []
grids_in_links_but_not_in_site = []
levels_in_site_but_not_in_links = []
levels_in_links_but_not_in_site = []
unique_grids_names = list(set(my_grid_names).symmetric_difference(set(link_grid_names)))
unique_levels_names = list(set(my_level_names).symmetric_difference(set(link_level_names)))
# print unique_grids_names
# print "**************************"
for grid_name in unique_grids_names:
    if grid_name in my_grid_names:
        grids_in_site_but_not_in_links.append(grid_name)
        continue
...
for level_name in unique_levels_names:
    if level_name in my_level_names:
        levels_in_site_but_not_in_links.append(level_name)
        continue
...
grids_in_site_but_not_in_links.sort(key = lambda x: x)
grids_in_links_but_not_in_site.sort(key = lambda x: x)
levels_in_site_but_not_in_links.sort(key = lambda x: x)
levels_in_links_but_not_in_site.sort(key = lambda x: x)
...
for grid_name in grids_in_site_but_not_in_links:
    ...
    # print link_doc.Title, monitoring_grid_id
    # monitoring_grid = link_doc.GetElement(monitoring_grid_id)
    # print monitoring_grid.Name

for level_name in levels_in_site_but_not_in_links:
