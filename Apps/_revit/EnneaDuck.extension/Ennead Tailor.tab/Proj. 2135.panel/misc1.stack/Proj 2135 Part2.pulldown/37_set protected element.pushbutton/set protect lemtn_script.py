...
__doc__ = "NOT IN USE"
__title__ = "37A_set protected element(grid, level, scopebox)(NOT IN USE)"
...
grids = list(grids)
grids.sort(key = lambda x: x.Name)
...
scopeboxs = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_VolumeOfInterest).WhereElementIsNotElementType().ToElements()
scopeboxs = list(scopeboxs)
scopeboxs.sort(key = lambda x: x.Name)



collection = grids + levels + property_lines + scopeboxs
...
need a nother function to show me what is current being record as protection in my file, and remove,
