...
def self_update_region_area(piece_type):
    ...
    if piece_type.LookupParameter("_TotalTargetIds").HasValue:
        ids =  piece_type.LookupParameter("_TotalTargetIds").AsString().split(";")
        elements = [DOC.GetElement(id) for id in ids]
        area = 0
        for element in elements:
            if element is None:
            ...
            area += element.LookupParameter("Area").AsDouble()  
        if area > 0:
            print ("Based on the total targets, the region area is {}".format(area))
            piece_type.LookupParameter("Total Area").Set(area)
...
def window_wall_ratio_sum(doc):
    ...
    for keyword in keywords:
        ...
        for i, piece_type in enumerate(piece_types):
            ...
            facade_glass_area_sum += glass_area
            # get the opaque area of the piece type
            total_area = piece_type.LookupParameter("Total Area").AsDouble()
    ...
    print ("Total area: {}".format(grand_total_area_total))
    ...
    grand_summery_type.LookupParameter("Total Area").Set(grand_total_area_total)
