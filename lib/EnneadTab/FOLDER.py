...
def copy_dir(source, dest, show_progress = False, progress_bar_note = "", ignore_keywords = [], allow_print_log = True):
...
def remap_filepath_to_folder(full_file_path):
    ...
    if USER.is_revit_beta_tester():
        address = "{}\\ENNEAD.extension".format(ENVIRONMENT.PUBLISH_BETA_FOLDER_FOR_REVIT)
    else:
        address = "{}\\ENNEAD.extension".format(ENVIRONMENT.PUBLISH_FOLDER_FOR_REVIT)

        
    return full_file_path.replace("{}\\ENNEAD.extension".format(ENVIRONMENT.WORKING_FOLDER_FOR_REVIT), address)
