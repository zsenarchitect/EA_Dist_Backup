...
"""
    ...
    def replace_old_lib_path(self):

        NEW_LIB_PATH = 'sys.path.append("..\lib")'
        OLD_LIB_PATH = r"sys.path.append(r'L:\4b_Applied Computing\03_Rhino\12_EnneadTab for Rhino\Source Codes\lib')"
        if OLD_LIB_PATH not  in self.content:
            return
        
        content = self.content.replace(OLD_LIB_PATH, NEW_LIB_PATH)
        self.save_content(content)


    def replace_old_EA(self):
        OLD_DEPEDENT_VALUE = "import EA_UTILITY as EA"
        NEW_DEPEDENT_VALUE = "import EA_UTILITY as EA\nimport EnneadTab"
...
def fix_EA_dependency_in_file(file_path):
    ...
    if  python_file.content:
        ...
        # python_file.replace_old_lib_path()
...
def fix_EA_dependency():
    ...
    root_folder = ENVIRONMENT.WORKING_FOLDER_FOR_RHINO
    ...
    for root, dirs, files in os.walk(root_folder):
        ...
        if root.startswith(ENVIRONMENT.CORE_MODULE_FOLDER_FOR_RHINO):
            continue
        # print (root)
    ...
    count = 0
    ceiling_limit = min(20, len(to_do_list))
