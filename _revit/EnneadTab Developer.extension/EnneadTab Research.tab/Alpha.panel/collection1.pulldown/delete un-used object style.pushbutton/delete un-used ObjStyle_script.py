...
__title__ = "delete un-used object srtyle"

from pyrevit import forms, DB, revit, script, output

def display_name(subC):
    return '{} --> {}'.format(subC.Parent.Name, subC.Name)

#classpyrevit._ExecutorParams


def num_of_changes_in_doc(e = DB.Events.DocumentChangedEventArgs):
    print (e), e.Cancellable
    deleted = e.GetDeletedElementIds(e).ToList()
    modified = e.GetModifiedElementIds(e).ToList()
    return deleted.Count + modified.Count

def process_subC(subC):

    t = DB.Transaction(revit.doc, "temp")
    t.Start()
    delete_subC(subC)
    print("##")
    print(DB.Events.DocumentChangedEventArgs)
    print(pyrevit.event_args)

    if num_of_changes_in_doc() > 0:
        print("in use, roll back")
        t.RollBack()
...
def delete_subC(subC):
    revit.doc.Delete(subC.Id)
    print("deleting")

################## main code below #####################
output = script.get_output()
output.close_others()
all_subCs = revit.query.get_subcategories(doc=revit.doc, purgable=True)

with revit.TransactionGroup("purge used object style"):
    for subC in all_subCs:
        print("-"*50)
        print(display_name(subC))
        process_subC(subC)
