...
def verify_selection(selected_elems, doc):
    if doc.IsFamilyDocument or True:
...
if verify_selection(selection, revit.doc):
    ...
    for sat_import in selection:
        ...
        for geo in sat_import.Geometry[geom_opts]:
            ...
            print(geo.GetSymbolGeometry())
            if isinstance(geo, DB.Solid) or True:
                for iObj in geo.SymbolGeometry:
			        if isinstance(iObj, DB.Solid):
				        solids.append(iObj)
                    #solids.append(geo.GetSymbolGeometry())
