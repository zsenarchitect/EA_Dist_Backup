...
if len(revit.get_selection()) == 0:
    ref_element = revit.pick_element("pick fllor")
else:
    ref_element = revit.get_selection()[0]

print(ref_element.UniqueId)

print(ref_element.Geometry[DB.Options()])
sk1 = revit.doc.GetElement(DB.ElementId(ref_element.Id.IntegerValue - 1) )
print(sk1)
ref_curvearrarray = sk1.Profile
print(ref_curvearrarray)
#print ref_element.SlabShapeEditor.Enable()
target_elements = revit.pick_elements("pick fllor")
with revit.Transaction("copy bound"):
    for el in target_elements:
        #sk = revit.doc.GetElement(DB.ElementId(el.Id.IntegerValue - 1) )
        #sk.Profile = ref_curvearrarray

        editor = el.SlabShapeEditor.Enable()
        #editor.DrawPoint
