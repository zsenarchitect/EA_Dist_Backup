...
__title__ = "Extract CAD by layer"
...
dwg = revit.get_selection()[0]
print(dwg)
opts = DB.Options()
opts.IncludeNonVisibleObjects = True

geos = dwg.Geometry[opts]
print(geos)
for geo in geos:
    print(geo)
    print(geo.GetType())
    print(geo.GetInstanceGeometry ())



print("^^^^^^^")
for para in dwg.Parameters:
...
IDEAS
    ...
    - export layer setting add a button to test display all the mapping key, and suggest people to run it again after renmaing
    ...
    - deploy many group to a draft view, zoom extend, export as image with group name being file name, to a folder,
"""
