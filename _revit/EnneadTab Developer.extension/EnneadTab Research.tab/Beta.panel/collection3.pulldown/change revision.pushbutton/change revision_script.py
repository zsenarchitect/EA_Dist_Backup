...
sheets = forms.select_sheets(title='Select Sheets')
revisions = forms.select_revisions()
revisions_id = [x.Id for x in revisions]
print(revision.Description)
for sheet in sheets:
    print(sheet.Name)
    #print sheet.GetCurrentRevision().Description
    sheet.SetAdditionalRevisionIds(List[DB.ElementId](revisions_id))
"""
