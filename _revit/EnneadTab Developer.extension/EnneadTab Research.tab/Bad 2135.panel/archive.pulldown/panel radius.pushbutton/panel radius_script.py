__doc__ = "pick one curve panel first, then run this tool, it will update the 'R' or 'r' parameter based on the curved host wall"
__title__ = "panel radius by host wall"

from pyrevit import forms, DB, revit, script

def find_similar_panels(panel):
    #print panel.Symbol
    #print panel.Symbol.FamilyName
    #print panel.Symbol.LookupParameter("Type Name").AsString()
    def is_match_panel(x):
        try:
            if panel.Symbol.FamilyName == x.Symbol.FamilyName and panel.Symbol.LookupParameter("Type Name").AsString() == x.Symbol.LookupParameter("Type Name").AsString():
                return True
            else:
                return False
...
def find_panel_host_wall_radius(panel):
    ...
    #curve = panel.Host.Location.Curve
    radius = panel.Host.get_Parameter(DB.BuiltInParameter.CURVE_ELEM_ARC_RADIUS).AsDouble()
    if radius == 0:
        print("Panel {} should be in curved wall".format(output.linkify(panel.Id)))
...
if len(selection) > 1:
    forms.alert("Please select 1 panel only.")
    script.exit()


if "Curtain Panels" not in selection[0].Category.Name:
    forms.alert("This is not a curtain panel.")
    script.exit()


panels = find_similar_panels(selection[0])
#print panels

with revit.Transaction("Panel get host wall radius"):
    try:
        map(lambda x: x.LookupParameter("R").Set(find_panel_host_wall_radius(x)), panels)
    except:
        map(lambda x: x.LookupParameter("r").Set(find_panel_host_wall_radius(x)), panels)

revit.get_selection().set_to(panels)
forms.alert("{} panels updated".format(len(panels)))
...
IDEAS
    ...
    - one panel to all panel , change R by host wall radius
