...
__title__ = "Unmonitored level grids"
...
def print_detail(list):
    for item in list:
        print("*"*5)
        print(item.Name)
        print(len(item.GetMonitoredLinkElementIds()))
        #print len(item.GetMonitoredLocalElementIds())
################## main code below #####################
...
print_detail(list(grids))
print_detail(list(levels))


#get levelsA from current file,

#get levelsB from selected link

#for each levelX in level A,
#get name,
#if find X name  in levelB list as Y,
    #check if Y is monitoring X, else alert inpdependent level, ask to set relationship
#else: alert to ask set relationship
