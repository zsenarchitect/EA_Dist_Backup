__doc__ = "Worth revisiting idea."
__title__ = "import material from link"
...
link_name_selected = forms.ask_for_one_item([x.GetLinkDocument().Title for x in revit_links], default = revit_links[0].GetLinkDocument().Title,prompt = "Pick the revit link.", title = "search materila in revit link")
...
selected_materials = forms.SelectFromList.show(link_materials,
                                                multiselect = True,
                                                name_attr = 'Name',
                                                button_name='Select Material to copy')
with revit.Transaction("import material"):
