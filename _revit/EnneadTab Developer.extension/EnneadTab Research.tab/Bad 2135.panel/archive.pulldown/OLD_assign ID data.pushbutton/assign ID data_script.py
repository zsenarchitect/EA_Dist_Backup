__doc__ = "xxx"
__title__ = "OLD do not use___Assign ID data by view or project"

from pyrevit import forms, DB, revit, script

def category_to_element(category, is_active_view_only = True):
    bic = DB.BuiltInCategory.Parse(DB.BuiltInCategory,category)
    #print bic
    if is_active_view_only:
        return DB.FilteredElementCollector(revit.doc, revit.doc.ActiveView.Id).OfCategory(bic).WhereElementIsNotElementType().ToElements()
    return DB.FilteredElementCollector(revit.doc).OfCategory(bic).WhereElementIsNotElementType().ToElements()
...
result = forms.alert(options = ["active view only.", "entire project."], msg = "The change is for [.....]")
if result == "active view only.":
    is_active_view_only = True
else:
    is_active_view_only = False
...
#type in target data
target_id = forms.ask_for_string(prompt="Target parameter Name = {0}\n\nType below the target ID for {0}".format(ID_para), title="where to ?")
#print target_id


#pick to assign categary: room, area, floor, wall, view, sheet, column, grid, ceiling
category = forms.SelectFromList.show(["Doors",\
                                    "Grids",\
                                    "Levels",\
                                    "Rooms",\
                                    "Floors",\
...
with revit.Transaction("Push ID data"):
    for cate in category:
        print("*"*20)
        current_elements = category_to_element(cate, is_active_view_only)
