...
__title__ = "Happy New Year"

from pyrevit import forms, DB, revit, script
from time import sleep

def update_trimmer(trimmer, step = 3):

    for i in range(step + 1):
        h_raw = 5000*(1-float(i)/step) + 50
        h = DB.UnitUtils.ConvertToInternalUnits(h_raw, DB.DisplayUnitType.DUT_MILLIMETERS)
        with revit.Transaction("frame update2"):
            trimmer.LookupParameter("h").Set(h)
        revit.uidoc.RefreshActiveView()
        sleep(0.2)

def play_door():
    GMs = DB.FilteredElementCollector(revit.doc).OfCategory(DB.BuiltInCategory.OST_GenericModel).WhereElementIsNotElementType().ToElements()
    #print GMs
    doors = filter(lambda x: x.Symbol.FamilyName == 'gate door', GMs)

    step = 12
    for i in range(step + 1):
        angle_raw = 90*float(i)/step
        angle = DB.UnitUtils.ConvertToInternalUnits(angle_raw, DB.DisplayUnitType.DUT_DECIMAL_DEGREES)

        with revit.Transaction("frame update"):

            for door in doors:
                door.LookupParameter("angle").Set(angle)
...
def play_words():
    ...
    #print masses
    trimmers = filter(lambda x: x.Symbol.FamilyName == 'trimmer', masses)
    #print trimmers
    trimmer_r = filter(lambda x: x.LookupParameter("Comments").AsString() == 'right', trimmers)[0]
    trimmer_l = filter(lambda x: x.LookupParameter("Comments").AsString() == 'left', trimmers)[0]
    trimmer_head = filter(lambda x: x.LookupParameter("Comments").AsString() == 'head', trimmers)[0]
    update_trimmer(trimmer_r)
    update_trimmer(trimmer_l)
    update_trimmer(trimmer_head)

################## main code below #####################
output = script.get_output()
output.close_others()

with revit.TransactionGroup("new year"):
