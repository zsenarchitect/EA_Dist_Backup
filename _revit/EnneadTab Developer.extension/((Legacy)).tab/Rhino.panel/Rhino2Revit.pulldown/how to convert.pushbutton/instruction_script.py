__doc__ = "Depreciated"
__title__ = "How to use it?(Depreciated)"
...
image = script.get_bundle_file("instruction_lock control.jpg")
...
image = script.get_bundle_file("instruction_3d in 2d.jpg")
output.print_image(image)


print("\n\n\n\n\n\n\n\n")
output.print_md ("##The default tool is the preferred method, it convert the most amount of content close to Revit freeform syntax. But sometimes you will see when Rhino 3dm file is decoded, not all solid can be read, 90% of time it is due to the Rhino geometry not being optimized. \nFor example, non-enclosed polysurface, very tight touch edge, non-manifold geometry or any other invalid geometry. For best result, please be familiar with the concept of G-continuity, Mesh Topology in Brep, etc.\n\nTry those steps to attempt to fix.")



print("\n\n1. curvature check: too much warpping is not welcomed in revit.")
image = script.get_bundle_file("instruction_curvature.jpg")
output.print_image(image)

print("\n\n2. non-manifold check: Should avoid, always.")
image = script.get_bundle_file("instruction_non manifold.jpg")
output.print_image(image)

print("\n\n3. valid and openness check: invalid geomtry should be fixed before carrying over to revit. A open polysurface/surface does NOT mean a failed revit convert, it really depends on design intent. ")
output.print_md("**But be aware that only closed polysurface/surface can be further joined/cut in revit enviornment.**")
image = script.get_bundle_file("instruction_valid and closed.jpg")
output.print_image(image)

print("\n\n4. rationalization check: Whenever possible, do not use the study model or competition model directly. Please try to rationalize your design before moving on to revit stage")
image = script.get_bundle_file("instruction_simplify ruled geomtry generation.jpg")
output.print_image(image)
...
image = script.get_bundle_file("instruction_zebra.jpg")
output.print_image(image)
...
image = script.get_bundle_file("instruction1.jpg")
output.print_image(image)

output.print_md ("\n\n##It is very important to use a Solid dwg option.")
image = script.get_bundle_file("instruction2.jpg")
output.print_image(image)

output.print_md ("\n\n##Then use this emergency convert and follow prompt.")
image = script.get_bundle_file("instruction_use force import.jpg")
output.print_image(image)
