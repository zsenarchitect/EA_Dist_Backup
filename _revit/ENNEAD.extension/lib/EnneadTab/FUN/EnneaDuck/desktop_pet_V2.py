...
def event(cycle,check,event_number,x):
    ...
    if event_number in idle_num:
        ...
        check = 1
        print('rotate')
        window.after(common_wait,update,cycle,check,event_number,x) #no. 5 = idle to swing
        ...
        print('swing')
        print (cycle)
        window.after(common_wait,update,cycle,check,event_number,x)#no. 10,11,12,13,15 = swing
    elif event_number == 14:
        check = 3
        print('shake')
        window.after(common_wait,update,cycle,check,event_number,x)#no. 15 = swing to idle
...
def update(cycle,check,event_number,x):
    ...
    #idle to swing
        ...
        frame = rotate[cycle]
        cycle ,event_number = gif_work(cycle,rotate,event_number,10,10)
    #swing
    elif check == 2:
        frame = swing[cycle]
        cycle ,event_number = gif_work(cycle,swing,event_number,10,15)
    #swing to idle
    elif check ==3:
        frame = shake[cycle]
        cycle ,event_number = gif_work(cycle,shake,event_number,1,1)
    #walk toward left
    elif check == 4:
...
def main():
    ...
    rotate = [tk.PhotoImage(file=impath+'rotate.gif',format = 'gif -index %i' %(i)) for i in range(57)]#idle to swing gif
    swing = [tk.PhotoImage(file=impath+'swing.gif',format = 'gif -index %i' %(i)) for i in range(21)]#swing gif
    shake = [tk.PhotoImage(file=impath+'shake.gif',format = 'gif -index %i' %(i)) for i in range(12)]#swing to idle gif
    walk_positive = [tk.PhotoImage(file=impath+'walking_left.gif',format = 'gif -index %i' %(i)) for i in range(9)]#walk to left gif
    walk_negative = [tk.PhotoImage(file=impath+'walking_right.gif',format = 'gif -index %i' %(i)) for i in range(9)]#walk to right gif
