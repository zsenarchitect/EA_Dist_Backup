...
"""
### TO-DO:
- Add text size and color options
- Auto-format display layers to remove [GFA] and {0.5} etc.
- Make the conduit class Singeleton so easier to call (Assigned to: **SZ**)
...
def local_msg(txt):
    try:# this is try-catch is temprory during moudle update perpoid, can be deleted in the future
        if EnneadTab.USER_CONSTANTS.is_SH_account():
            print (txt)
            return
        EnneadTab.NOTIFICATION.messenger(main_text = txt)
    except:
        print(txt)
...
class EA_GFA_Conduit(Rhino.Display.DisplayConduit):
    ...
    def DrawForeground(self, e):
        ...
        recent_time_text = EnneadTab.TIME.get_formatted_time(sc.sticky["reset_timestamp"])
