...
def export_from_masterplan():
    ...
    for group in rs.GroupNames():
        ...
        if plot_name in used_plots:
            EnneadTab.NOTIFICATION.toast(main_text = "Skipping plot {}. File already been claimmed.".format(plot_name))
            print(plot_name)
            continue
        """
        if os.path.exists(filepath):
            EnneadTab.NOTIFICATION.toast(main_text = "Skipping plot {}. File already exists".format(plot_name))
            continue
        """
