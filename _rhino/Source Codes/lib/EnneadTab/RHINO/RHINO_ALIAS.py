...
def register_alias_set():
    ...
    for root, dirs, files in os.walk(ENVIRONMENT_CONSTANTS.RHINO_SCRIPT_FOLDER):
        for file in files:
            if file.endswith(".py"):
                ...
                alias_list = data.get('__alias__')
                if not isinstance(alias_list, list):
                    alias_list = [alias_list]

                for alias in alias_list:
                    if rs.IsAlias(alias) and "EnneadTab-for-Rhino" not in exisitng_alias:
