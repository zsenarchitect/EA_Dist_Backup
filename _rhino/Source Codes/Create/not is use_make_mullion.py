...
def make_mullion(profile = None, base_srf = None, use_profile_bounding = False,  use_rs = True):
    ...
    top_crv = rs.DuplicateSurfaceBorder(offset_srf, type = 1)
    ...
    for profile_element in profile_on_crv:
        ...
        if use_rs:
            ...
            rs.SelectObjects([profile_element,base_crv, top_crv])

            #.format(rs.coercegeometry(base_crv), rs.coercegeometry(profile_element))
            rs.Command("_NoEcho _Sweep2  _Style=_Roadlike _UntrimmedMiters=_Yes _EnterEnd", echo = False)
