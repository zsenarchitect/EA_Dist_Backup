__doc__ = "Rename scopebox by removing all '_' in scopebox name"
__title__ = "rename scopebox"
...
scopeboxs = DB.FilteredElementCollector(revit.doc).OfCategory(DB.BuiltInCategory.OST_VolumeOfInterest).WhereElementIsNotElementType().ToElements()


with revit.Transaction("rename scopebox"):
    for scopebox in scopeboxs:
        print("*"*10)
        print(scopebox.Name)
        scopebox.Name = scopebox.Name.replace("_", "")
        print(scopebox.Name)
