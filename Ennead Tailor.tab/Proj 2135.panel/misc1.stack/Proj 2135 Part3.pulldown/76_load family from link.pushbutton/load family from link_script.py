__doc__ = "Legacy, now in family panel"
__title__ = "76_load 2D family from container link(Legacy)"
...
def get_link_doc():
    ...
    for link_doc in all_docs:
        if not hasattr(link_doc, "Title"):
            continue
        #print link_doc.Title
        if link_doc.Title == "Detail Component Family_Container":
            return link_doc
    return None
...
def load_families():
    ...
    #t.Start()
    map(lambda link_family: load_family_to_link(doc, link_family), link_families)
    #t.Commit()
