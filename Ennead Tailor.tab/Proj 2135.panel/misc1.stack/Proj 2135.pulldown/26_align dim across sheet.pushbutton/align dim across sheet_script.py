...
def get_dims_in_view(view):
    dims = DB.FilteredElementCollector(doc, master_view.Id).OfCategory(DB.BuiltInCategory.OST_Dimensions).WhereElementIsNotElementType().ToElements()
    return filter(lambda x: x.DimensionShape == DB.DimensionShape.Linear, dims)
...
def process_view(view):
    def vector_from_crv(input_crv):
        # project down to view first
        crv = input_crv.MakeBound(0,1)

        start = crv.GetEndPoint(0)
        end = crv.GetEndPoint(1)

        vector = DB.XYZ(end - start)
    ...
    def get_dist_between_dim(dim, other_dim):

        vector = vector_from_crv(dim.Curve)
        other_vector = vector_from_crv(other_dim.Curve)
        dist = False
        if vector.AngleTo(other_vector) != 0:
            return dist
    ...
    # get all dims in this view
    current_dims = get_dims_in_view(view)

    # for dim in master_dims, find parrell dim in current view
    for master_dim in master_dims:
        for other_dim in current_dims:
            dist = get_dist_between_dim(master_dim, other_dim)
            if not dist:
                continue
            if dist < tolerance:
                print("very close")
...
# Pick other views to catch up, exclude master view
views = forms.select_views(multiple = True)
for view in views:
    if view.Name == master_view.Name:
        views.pop(view)

# List of dim type to ignore
# to be added

# Get dim in master view and get infinite curve
master_dims = get_dims_in_view(master_view)

#
# Each curve project other every curve in other view, if distance valid that is parallel, if distances smaller than tolerance then it is a match, then set dim curve to master curve while keeping all references
t = DB.Transaction(doc, "match dims location")
t.Start()
