...
def get_data_from_record_file():
    ...
    file_name = FILE_NAME
    if EnneadTab.FOLDER.is_file_exist_in_folder(file_name, EnneadTab.FOLDER.get_EA_local_dump_folder()):
...
def kill_record():
    ...
    if not data:
        EnneadTab.NOTIFICATION.messenger(main_text = "No Active Record Found.")
    ...
    EnneadTab.DATA_FILE.save_dict_to_json(data, get_record_file_path())
...
def is_doc_opened(doc):
    ...
    if doc.Title in data.keys():
        EnneadTab.NOTIFICATION.toast(main_text = "Wait a minutes...", sub_text = "This document seems to be opened already.")
        EnneadTab.SPEAK.speak("Unless you recently crashed Revit, this document seems to be opened already. You should prevent opening same file on same machine, because this will confuse central model which local version to allow sync.")
        EnneadTab.REVIT.REVIT_FORMS.notification(main_text = "This document seems to be opened already in other session.\nOr maybe recently crashed.\nAnyway, I have detected the record already existing.",
...
def is_hate_sync_monitor():
    ...
    """
    return EnneadTab.DATA_FILE.get_revit_ui_setting_data(("radio_bt_sync_monitor_never", False))
