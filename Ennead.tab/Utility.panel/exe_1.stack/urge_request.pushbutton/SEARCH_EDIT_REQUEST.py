...
def is_there_edit_request(request_icon_file):
    try:
        #request_icon = pyautogui.locateOnScreen(request_icon_file, confidence = 0.8)
        request_icon = pyautogui.locateOnScreen(request_icon_file)
        #print request_icon
        if not request_icon:
            return False
...
def is_another_REQUEST_LOOKUP_running():

    #print [x.title for x in pyautogui.getAllWindows()]
    for window in pyautogui.getAllWindows():
        #print window.title
        if window.title == u"EnneadTab Revit Request Lookup":
            return True
    return False
...
def main():
    if is_another_REQUEST_LOOKUP_running():
    ...
    pygame.display.set_caption("EnneadTab Revit Request Lookup")
    ...
    request_icon_file = "{}\\images\\icon_lookup.png".format(exe_folder)
    ...
    while run:
        ...
        if count_down_found < 0 :
            ...
            if life_count % (5 * FPS) == 0 and is_there_edit_request(request_icon_file):
                ...
                EA_TOASTER.toast(message = "hmmm.", title = lines[0], app_name = "EnneadTab Monitor", icon = None, click = None, actions = None)
        ...
        draw_text("EnneadTab Revit Edit Request Lookup. ", font, TEXT_COL, 50, 50)
        ...
        draw_text("This tool actively look for", font, TEXT_COL_HIGHTLIGHT, 50, 320)
        draw_text("the edit request icon on your screen.", font, TEXT_COL_HIGHTLIGHT, 50, 340)
        ...
        draw_text("To save memory, EnneadTab Request Lookup will close itself in {}m {}s if there is nothing to monitor.".format(text_min, text_secs), font_note, TEXT_COL, 50, SCREEN_HEIGHT - 20)
        life_count -= 1
...
try:
  ...
  with open(r"{}\error_log.txt".format(exe_folder), "w") as f:
      f.write(error)
      EA_UTILITY.open_file_in_default_application(r"{}\error_log.txt".format(exe_folder))
